...
public abstract class DateSlider extends Dialog {
	...
	protected void setTitle() {
		if (mTitleText != null) {
			mTitleText.setText(getContext().getString(R.string.dateSliderTitle) + String.format(": %te. %tB %tY", mTime, mTime, mTime));
		}
	}
	...
	public abstract class Labeler {
		...
		/**
		 * returns a new TimeTextView instance, is only called a couple of times
		...
		/**
		 * This method will be called constantly, whenever new date information
		 * is required it receives a timestamps and adds "val" time units to
		 * that time and returns it as a TimeObject
		 * 
		 * @param time
		 * @param val
		 * @return
