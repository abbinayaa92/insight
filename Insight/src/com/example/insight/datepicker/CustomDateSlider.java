...
package com.example.insight.datepicker;
...
import com.example.insight.datepicker.TimeView.TimeTextView;
...
public class CustomDateSlider extends DateSlider {
	...
	public void onCreate(Bundle savedInstanceState) {
		// this needs to be called before everything else to set up the main
		// layout of the DateSlider
		super.onCreate(savedInstanceState);

		LayoutInflater inflater = (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		LayoutParams lp = new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT);

		// create the year scroller and assign its labeler and add it to the
		// layout
		...
		// create the month scroller and assign its labeler and add it to the
		// layout
		...
		// this method _has_ to be called to set the onScrollListeners for all
		// the Scrollers
	...
	protected Labeler yearLabeler = new Labeler() {
		...
		/**
		 * add "val" year to the month object that contains "time" and returns
		 * the new TimeObject
		 */
		@Override
	...
	// the month labeler takes care of providing each TimeTextView element in
	// the monthScroller
	// with the right label and information about its time representation
	protected Labeler weekLabeler = new Labeler() {

		/**
		 * add "val" months to the month object that contains "time" and returns
		 * the new TimeObject
		 */
		@Override
		public TimeObject add(long time, int val) {
	...
	protected Labeler dayLabeler = new Labeler() {
		...
		/**
		 * add "val" days to the month object that contains "time" and returns
		 * the new TimeObject
		 */
		@Override
		public TimeObject add(long time, int val) {
	...
	protected void setTitle() {
		if (mTitleText != null) {
			mTitleText.setText(getContext().getString(R.string.dateSliderTitle) + String.format(": %tA, %te/%tm/%ty", mTime, mTime, mTime, mTime));
		}
	}
